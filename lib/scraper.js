            ...
            if(err) {
                ...
                cb(err, select(dom, '#ssi0 tr').map(function(elem) {
                    ...
                    return {
                        ...
                        revenue: {
                            amount: parseInt(content(select(elem, '.cnncol4')[0]), 10),
                            change: parseInt(content(select(elem, '.cnncol5')[0]), 10)
                        },
                        profit: {
                            amount: parseInt(content(select(elem, '.cnncol6')[0]), 10),
                            change: parseInt(content(select(elem, '.cnncol7')[0]), 10)
                        }
                    };
                }).filter(funkit.id));
            }
